...
export class RegistrationController extends BaseController {
    ...
    public async registerNewUser(
        ...
        try {
            const registrationResult = await ServiceRegistry.instance
                .getService(RegistrationService)
                .registerUser(login, password, request.body.unlinkedSocialUser);

            if (registrationResult.registrationStatus === RegistrationStatus.OK) {
                SecurityHelper.setJWTHeader(response, registrationResult.newAccessToken);
            }
    ...
    public async confirmRegistrationByEmail(@Param("token") token: string, @Req() request: Request, @Res() response: Response) {
        SecurityHelper.setCurrentUserAnonymous(response);
    ...
    public async confirmRegistrationByCode(@Param("code") code: number, @Req() request: Request, @Res() response: Response) {
        try {
            const sessionUser = SecurityHelper.getSessionUserFromToken(request);
            const result = await ServiceRegistry.instance
                .getService(RegistrationService)
                .confirmRegistrationByCode(code, sessionUser.appUserId);
