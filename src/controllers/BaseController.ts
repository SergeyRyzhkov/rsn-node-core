...
export class BaseController {
    public createSuccessResponseWithMessage(
        result: any,
        res: Response,
        statusCode = res.statusCode,
        message: ClientNotifyMessage,
        redirectUrl?: string
    ) {
    ...
    public createRedirectResponse(response: Response, location: string) {
        ...
        return this.createSuccessResponse({}, response.location(loc), 301);
    }
}
    ...
    return createParamDecorator({
        ...
        value: (action) => {
            return {
                ...
                pagination: {
                    offset:
                        action.request.query.offset != null && action.request.query.offset !== "undefined"
                            ? action.request.query.offset
                            : 0,
                    limit:
                        action.request.query.limit != null && action.request.query.limit !== "undefined"
                            ? action.request.query.limit
                            : 20,
                },
            };
        },
