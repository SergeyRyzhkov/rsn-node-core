...
export class ResetPasswordService extends BaseService {
    ...
    constructor(options?: ResetPasswordOptions) {
        super();
        this.options =
            options || ConfigManager.instance.getOptionsAsClass(SecurityConfig, "SecurityConfig")?.resetPasswordOptions;
    ...
    public async sendResetPasswordMessage(login: string) {
        ...
        if (!!appUser && appUser.appUserBlockedInd === 1) {
            return result.makeFailed("Учетная запись заблокирована");
        }
    ...
    public async confirmResetPasswordByCode(code: string) {
        ...
        try {
            ...
            const isExpared =
                Date.now() - Date.parse(appUser.appUserResetPwdDate) > this.options.resetPasswordLifetimeInSeconds * 1000;
